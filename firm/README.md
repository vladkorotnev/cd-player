...
* *NEC PC-CD400D/4*: kinda works, very unstable, similar to the CDR-1400 but a bit better. Changer functionality kinda works. SPDIF works. Would be perfect if it were running more stable. Enabling SCAN gets the BSY flag stuck until completed, so for now configured with SoftSeek.
